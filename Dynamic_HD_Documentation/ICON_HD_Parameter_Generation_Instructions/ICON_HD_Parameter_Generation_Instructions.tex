...
\begin{figure}
%\includegraphics[angle=90,width=\textwidth,height=0.98\textheight]{low_res_icon_proc_with_sinks.pdf}
\caption{}
\label{fig-low-res-icon-proc-with-sinks}
\end{figure}

\begin{figure}
%\includegraphics[angle=90,width=\textwidth,height=0.98\textheight]{low_res_icon_proc_with_sinks_var.pdf}
\caption{}
...
\begin{figure}
\includegraphics[angle=90,width=\textwidth,height=0.98\textheight]{high-res-icon-proc.pdf}
\caption{}
...
%
%You can examine the main routines of the script in 
%DynamicHD/Dynamic_HD_bash_scripts/generate_icon_hdpara_top_level_driver.sh
%and 
%DynamicHD/Dynamic_HD_Scripts/Dynamic_HD_Scripts/create_icon_coarse_river_directions_driver.py
%
...
\end{lstlisting}
The script will produce a number of files. The key output file is the icon hdpara file  \lstinline[style=bash_input]{output_icon_hdpara.nc} for input into the ICON model.% producehdpara_r2b4_with_selected_sinks.nc (the actual file for the model) and  hdcatch_r2b4_with_selected_sinks.nc (the catchments) as output files. (The output cumulative flow filename doesn?t do anything yet - that feature will be added soon.) The command line arguments are as follows:
%1 - input_orography_filepath - input orography file on ICON grid
%2 - input_ls_mask_filepath - input lsmask file on ICON grid
%3 - output_hdpara_filepath - output hdpara file 
%4 - output_catchments_filepath - output catchments file
%5 - output_accumulated_flow_filepath - dummy, does nothing at the moment
%6 - config_filepath - a file specifying options for the top level bash script i.e. icon_rdir_gen_top_level_config.cfg
%7 - working_directory - for the script
%8 - cell_numbers - mapping of icon cells onto 10 min lat-lon. This is actually generated by one of the tools used in the script as an optional extra but that would require extra steps so it is easiest just to download my version for the moment
%9 - grid_file - icon grid file
%10 - cotat_params_file - some upscaling parameters for the upscaling algorithm. An appropriate file is provided in my scripts
%11 - compilation_required - Needs to be set to ?true? the first time you run but you can later set it to ?false? to make the script run quicker
%12 - true_sinks_filepath (optional) - A file containing the position of false sinks to use on a 10min lat-lon grid. You could start from the file I suggested above for this but you will probably want to further develop this yourself. If you don?t specify this then no true sinks will be used when generating the set of river directions for Asia, North America and Europe).
%
%You can adjust the endorheic/internal basins for Europe, Asia and North America by adjusting the true sink file. To add an new internal basin just find the lowest point of the depression the basin occupies in the 10min lat-lon orography corrected_orog_intermediary_ICE5G_and_tarasov_upscaled_srtm30plus_north_america_only_data_ALG4_sinkless_glcc_olson_lsmask_0k_20170517_003802.nc and put a true flag there in the true sinks file.
%
%To adjust which internal basins are kept from the hydrosheds river directions look in the 10 minute catchment file a run using all internal basins (such a file is available at /home/mpim/m300468/mk_ic_rdirs/ten_minute_catchments_all_sinks.nc).  Choose the endorheic  catchments you want to keep and write down their catchment number and then specify these in a comma separated list in icon_hdpara_generation_driver.cfg after exclude_catchments= (the file current has an example of this). All other internal basins from the hydrosheds river directions will be removed and only these will be kept (you can also choose only to remove a selected list of catchments with the replace_only_catchments option or keep all internal basins with keep_all_internal_basins=False).  
%
%There are two different options for how endorheic catchments that are removed are replaced. The simplest is to redirect the water to the sea (though it may end up in a bigger endorheic basin in some cases anyway) - this is selected with replace_internal_basins_with_rdirs_with_truesinks=False. The other option is to set replace_internal_basins_with_rdirs_with_truesinks=True. This is a more scientifically justified but a bit more complicated to use. You need to specify a true sink point for each major endorheic catchment - this isn?t not used to generated the major endorheic catchment itself (assuming you have excluded it from being removed as detailed above) but is used to reconnect other minor endorheic catchments with this major endorheic catchment in a logical way (i.e. across the lowest rims of basins). 
%
...
%cdo expr,"lsm=(MASK==1 || MASK==2)" hdpara_r2b4_with_selected_sinks.nc mask_from_para.nc
%cdo diff mask_from_para.nc maxlnd_lsm_013_0031 
%Which should result in no differences? occasionally there a problems with endorheic catchments next to the coast? if this happens let me know and I?ll send instructions how to correct this by hand.
\end{enumerate}
