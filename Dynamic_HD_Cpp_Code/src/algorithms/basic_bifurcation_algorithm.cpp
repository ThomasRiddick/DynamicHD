...
basic_bifurcation_algorithm::~basic_bifurcation_algorithm() {
...
basic_bifurcation_algorithm_latlon::~basic_bifurcation_algorithm_latlon() {
  for (int i = 0; i< maximum_bifurcations;i++) delete bifurcation_rdirs[i];
  delete working_rdirs; delete master_rdirs;
}

basic_bifurcation_algorithm_icon_single_index::
  ~basic_bifurcation_algorithm_icon_single_index() {
...
void basic_bifurcation_algorithm::setup_fields(int* cumulative_flow_in,
...
void basic_bifurcation_algorithm::bifurcate_river(pair<coords*,vector<coords*>> river){
  main_channel_mask->set_all(not_main_channel);
  major_side_channel_mask->set_all(false);
...
void basic_bifurcation_algorithm::find_shortest_path_to_main_channel(coords* mouth_coords){
  completed_cells->set_all(false);
  connection_found = false;
  q.push(new landsea_cell(mouth_coords->clone()));
  while (!q.empty()){
    cell* center_cell = q.front();
    q.pop();
...
void basic_bifurcation_algorithm::process_neighbors()
{
...
inline void basic_bifurcation_algorithm::process_neighbor()
{
  ...
  if (!( (*major_side_channel_mask)(nbr_coords) ||
    ...
    if ((*main_channel_mask)(nbr_coords) == main_channel_valid) {
      ...
      //repurposed for use in this algorithm)
...
void basic_bifurcation_algorithm::track_main_channel(coords* mouth_coords){
  cells_from_mouth = 0;
  q.push(new landsea_cell(mouth_coords->clone()));
...
//No longer providing option for non-diagonal neighbors only as it never gets used
void basic_bifurcation_algorithm::
...
inline void basic_bifurcation_algorithm::process_neighbor_track_main_channel() {
  coords* nbr_coords = neighbors_coords->back();
  neighbors_coords->pop_back();
...
inline void basic_bifurcation_algorithm::process_neighbor_find_highest_flow() {
  coords* nbr_coords = neighbors_coords->back();
  neighbors_coords->pop_back();
  int cumulative_flow_at_nbr_coords = (*cumulative_flow)(nbr_coords);
  if( cumulative_flow_at_nbr_coords  > highest_cumulative_flow_nbrs  ) {
...
inline void basic_bifurcation_algorithm_latlon::transcribe_river_direction(coords* coords_in){
  (*master_rdirs)(coords_in) = (*working_rdirs)(coords_in);
}

inline void basic_bifurcation_algorithm_icon_single_index::transcribe_river_direction(coords* coords_in){
  (*master_next_cell_index)(coords_in) = (*working_next_cell_index)(coords_in);
}

void basic_bifurcation_algorithm_latlon::mark_bifurcated_river_direction(coords* initial_coords,
...
void basic_bifurcation_algorithm_icon_single_index::mark_bifurcated_river_direction(coords* initial_coords,
                                                                         coords* destination_coords){
...
inline bool basic_bifurcation_algorithm_latlon::cell_flows_into_cell(coords* source_coords,
                                                                     coords* destination_coords){
...
inline bool basic_bifurcation_algorithm_icon_single_index::cell_flows_into_cell(coords* source_coords,
                                                                                coords* destination_coords){
  auto destination_generic_1d_coords =
    static_cast<generic_1d_coords*>(destination_coords);
...
void basic_bifurcation_algorithm_latlon::mark_river_direction(coords* initial_coords,
                                                       coords* destination_coords){
  (*working_rdirs)(initial_coords) = _grid->calculate_dir_based_rdir(initial_coords,destination_coords);
}

void basic_bifurcation_algorithm_icon_single_index::
...
inline coords* basic_bifurcation_algorithm_latlon::get_next_cell_downstream(coords* initial_coords) {
...
inline coords* basic_bifurcation_algorithm_icon_single_index::
...
void basic_bifurcation_algorithm_latlon::reset_working_flow_directions(){
  working_rdirs->set_all(0.0);
}

void basic_bifurcation_algorithm_icon_single_index::reset_working_flow_directions(){
  working_next_cell_index->set_all(-1);
}

void basic_bifurcation_algorithm_latlon::setup_fields(map<pair<int,int>,
                                                      ...
                                                      bool* landsea_mask_in,
                                                      grid_params* grid_params_in){
  basic_bifurcation_algorithm::setup_fields(cumulative_flow_in,
...
void basic_bifurcation_algorithm_icon_single_index::setup_fields(map<int,vector<int>> river_mouths_in,
...
double* basic_bifurcation_algorithm_latlon::get_bifurcation_rdirs(){
...
int* basic_bifurcation_algorithm_icon_single_index::get_bifurcation_next_cell_index(){
  int array_size = _grid->get_total_size();
...
int basic_bifurcation_algorithm_latlon::get_maximum_bifurcations(){
  return maximum_bifurcations;
}

int basic_bifurcation_algorithm_icon_single_index::get_maximum_bifurcations(){
  return maximum_bifurcations;
}
