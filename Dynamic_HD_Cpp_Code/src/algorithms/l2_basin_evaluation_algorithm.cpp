...
void basin_evaluation_algorithm::evaluate_basin(){
	...
	while(true){
		...
		if (new_center_cell_height <= surface_height) {
			if (new_center_cell_height < surface_height) {
				if (lake->connection_set) {
						if (! find_specific_lake_on_level(q,
				                 									 		center_coords,
				                 									 		center_cell_height,
				                 									 		lake->connects_to_lake)) throw runtime_error()
						throw runtime_error()
						//SETUP NEW LAKE + MINIMA --> CENTER CELL
						lake* new_lake = new lake()
						lakes->push(new_lake);
						basin_connections.make_new_link(basin_number,root_target_basin_number);
				} else {
					lake->set_exit(new_center_coords);
				}
			} else {
				int target_lake_number;
					throw runtime_error();
					//LOOK FOR AN EXIT - CONNECTED OR FILLED AND NOT IN THIS BASIN
				if (find_any_outflow_on_level(q,
									   							    center_coords
									   							    center_cell_height,
									   							    target_lake_number,
									   							    outflow_coords)){
					lakes->get_lake_by_number(target_lake_number);
					if (! target_lake->connection_set){
						target_lake->set_connection(basin_number);
						throw runtime_error();
						//SETUP NEW LAKE + MINIMA --> CENTER CELL
						lake* new_lake = new lake()
						lakes->push(new_lake);
