...
cd -
echo "Running Version ${version} of the Dynamic Lake Spinup Code"
...
present_day_base_orography_filepath=${3}
glacier_mask_filepath=${4}
output_hdpara_filepath=${5}
ancillary_data_directory=${6}
working_directory=${7}
input_hdstart_filepath=${8}
output_lakepara_filepath=${9}
output_hdstart_filepath=${10}
output_lakestart_filepath=${11}
output_binary_lake_mask_filepath=${12}
date=${13}
additional_corrections_list_filepath=${14}

#Check number of arguments makes sense
if [[ $# -ne 14 ]]; then
  echo "Wrong number of positional arguments ($# supplied), script only takes 14" 1>&2
  exit 1
fi
...
if ! [[ ${output_hdstart_filepath##*.} == "nc" ]] ; then
...
if ! [[ -e $input_hdstart_filepath ]]; then
...
input_lake_volumes_filepath=${working_directory}/lakestart_zero.nc

cdo const,0.0,${input_orography_filepath} ${input_lake_volumes_filepath}
...
${source_directory}/bin/hd_and_lake_model hdpara_instant_throughflow.nc ${ancillary_data_directory}/hdstart_zeros.nc 365 ${working_directory} ${ancillary_data_directory}/spinup_run_lake.ctl ${ancillary_data_directory}/cell_areas_on_surface_grid.nc
