...
contains
    ...
    subroutine flow_latlon(input_fine_river_directions,input_fine_total_cumulative_flow,&
        ...
        type(latlon_section_coords) :: original_coarse_section_coords
        ...
        integer :: value
            ...
            allocate(output_coarse_river_directions_indices(nlat_coarse+2,nlon_coarse,2))
            yamazaki_section_coords = latlon_section_coords(1,1,nlat_fine+2*scale_factor,nlon_fine)
            original_coarse_section_coords = latlon_section_coords(2,1,nlat_coarse+1,nlon_coarse)
            ...
            do j = 1,nlon_coarse
                do i = 2,nlat_coarse+1
                    cell_section_coords = latlon_section_coords((i-1)*scale_factor+1,(j-1)*scale_factor+1,scale_factor,scale_factor)
                    dir_based_rdirs_cell = latlon_dir_based_rdirs_cell(cell_section_coords, &
                        expanded_input_fine_river_directions, expanded_input_fine_total_cumulative_flow, &
                        yamazaki_outlet_pixels,yamazaki_section_coords)
                    call dir_based_rdirs_cell%yamazaki_calculate_river_directions_as_indices&
                        (output_coarse_river_directions_indices,original_coarse_section_coords)
                    call dir_based_rdirs_cell%destructor()
                end do
            end do
            ...
            output_coarse_river_directions_lat_index = &
                output_coarse_river_directions_indices(2:nlat_coarse+1,1:nlon_coarse,1)
            output_coarse_river_directions_lon_index = &
            output_coarse_river_directions_indices(2:nlat_coarse+1,1:nlon_coarse,2)
