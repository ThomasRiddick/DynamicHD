...
subroutine create_info_dump(this,water_to_lakes,lake_volumes,_NPOINTS_HD_)
  class(lake_logger), intent(in) :: this
  real(dp), dimension(_DIMS_), pointer, intent(in) :: water_to_lakes
  real(dp), dimension(:), pointer, intent(in) :: lake_volumes
  ...
  ! Not implemented
  _ELSE_
  integer :: lat_dimid_hd,lon_dimid_hd
  _END_IF_USE_SINGLE_INDEX_
  integer, dimension(2) :: dimids_hd
    if(this%timestep == info_dump_timestep) then
        call check_return_code(nf90_create(info_dump_filename,nf90_noclobber,ncid))
        _IF_USE_SINGLE_INDEX_
        ! Not implemented
        _ELSE_
        call check_return_code(nf90_def_dim(ncid, "lat",nlat_hd,&
                                            lat_dimid_hd))
        call check_return_code(nf90_def_dim(ncid, "lon",nlon_hd,&
                                            lon_dimid_hd))
        dimids_hd = (/lat_dimid_hd,lon_dimid_hd/)
        _END_IF_USE_SINGLE_INDEX_
...
subroutine log_process(this,lake_number,active_lake, &
                       lake_type,lake_volume,process_name)
  class(lake_logger), intent(inout) :: this
  integer, intent(in) :: lake_number
  logical, intent(in) :: active_lake
  integer, intent(in) :: lake_type
  real(dp), intent(in) :: lake_volume
  ...
  character(len=30) :: process_name_fixed_length
    ...
    if(this%logging_status) then
      ...
      this%lake_process_log(this%log_position,2) = int(active_lake)
      this%lake_process_log(this%log_position,3) = lake_type
  ...
  integer :: i
    if (this%log_position > 0) then
      do i = 1,this%log_position
        write(*,'(3(X,I8),3X,ES14.7,3X,A)') this%lake_process_log(i,1),this%lake_process_log(i,2),&
                   this%lake_process_log(i,3),this%lake_volumes(i),this%process_names(i)
      end do
    end if
...
subroutine log_process_wrapper(lake_number,active_lake,&
                               lake_type,lake_volume,process_name)
  integer, intent(in) :: lake_number
  logical, intent(in) :: active_lake
  integer, intent(in) :: lake_type
  real(dp), intent(in) :: lake_volume
  character(len=*), intent(in) :: process_name
    call global_lake_logger%log_process(lake_number,active_lake, &
                                        lake_type,lake_volume,process_name)
end subroutine log_process_wrapper

subroutine create_info_dump_wrapper(water_to_lakes,lake_volumes, &
                                    _NPOINTS_LAKE_,_NPOINTS_HD_)
  real(dp), dimension(_DIMS_),pointer :: water_to_lakes
