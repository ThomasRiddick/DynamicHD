...
subroutine init_lake_model(lake_model_ctl_filename,initial_spillover_to_rivers, &
  ...
  real(dp) :: step_length
    ...
    global_lake_model_parameters%water_to_hd(_DIMS_) = &
      global_lake_model_parameters%water_to_hd(_DIMS_)/global_step_length
    lake_interface_fields%water_from_lakes(_DIMS_) = global_lake_model_parameters%water_to_hd(_DIMS_)
    deallocate(initial_water_to_lake_centers)
end subroutine init_lake_model

subroutine init_lake_model_test(lake_parameters,lake_parameters_as_array, &
                                initial_water_to_lake_centers, &
                                lake_interface_fields,step_length)
  ...
  type(lakeinterfaceprognosticfields), intent(inout) :: lake_interface_fields
  real(dp) :: step_length
    global_lake_model_parameters => lake_parameters
...
subroutine run_lake_model(lake_interface_fields)
  type(lakeinterfaceprognosticfields), intent(inout) :: lake_interface_fields
    global_lake_model_parameters%water_to_lakes(_DIMS_) = &
      global_lake_model_parameters%water_to_lakes(_DIMS_)*global_step_length
    call run_lakes(global_lake_model_parameters,global_lake_model_prognostics)
    if (global_run_water_budget_check) then
      call check_water_budget(global_lake_model_prognostics)
    end if
...
subroutine run_lake_model_jsbach(water_to_lakes_in,water_to_hd_out)
  ...
  real(dp), allocatable, dimension(_DIMS_), intent(out)  :: water_to_hd_out
    global_lake_model_parameters%water_to_lakes(_DIMS_) = water_to_lakes_in(_DIMS_)
    call run_lakes(global_lake_model_parameters,global_lake_model_prognostics)
    water_to_hd_out(_DIMS_) = global_lake_model_parameters%water_to_hd(_DIMS_)
...
!   integer :: nlat
!     nlat = global_lake_model_parameters%nlat_surface_model
! end function get_surface_model_nlat

! function get_surface_model_nlon() result(nlon)
!   integer :: nlon
!     nlon = global_lake_model_parameters%nlon_surface_model
