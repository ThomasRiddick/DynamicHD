...
import configparser
import numpy as np
from Dynamic_HD_Scripts.base.iodriver import get_file_extension
...
def drive_orography_upscaling(input_fine_orography_file,output_coarse_orography_file,
    ...
    **coarse_grid_kwargs: keyword dictionary; the parameters of the coarse grid to upscale
    ...
    if landsea_file:
        landsea_mask = iodriver.load_field(landsea_file,
        ...
        landsea_mask = field.makeEmptyField(field_type='Generic',dtype=np.int32,
                                            grid_type=fine_grid_type,**fine_grid_kwargs)
    if true_sinks_file:
        true_sinks = iodriver.load_field(true_sinks_file,
